			    ...
			    int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		/*
		 * Default speaker path: feed I2S3 TX (TFA9874 smart amps)
		 * from DL1. Downstream relies on the Android audio HAL
		 * setting the "I2S3_CHx" mixer kcontrols per stream; with no
		 * HAL on mainline, program the interconnection bits here on
		 * every BE power-up so playback reaches the amps without
		 * userspace routing setup. Bits drop again at power-down,
		 * matching the mixers' AUTODISABLE behaviour; userspace can
		 * still route differently via the kcontrols.
		 */
		if (i2s_priv->id == MT6895_DAI_I2S_3) {
			regmap_set_bits(afe->regmap, AFE_CONN0,
					BIT(I_DL1_CH1));
			regmap_set_bits(afe->regmap, AFE_CONN1,
					BIT(I_DL1_CH2));
		}
		break;
	case SND_SOC_DAPM_POST_PMD:
		...
		if (i2s_priv->id == MT6895_DAI_I2S_3) {
			regmap_clear_bits(afe->regmap, AFE_CONN0,
					  BIT(I_DL1_CH1));
			regmap_clear_bits(afe->regmap, AFE_CONN1,
					  BIT(I_DL1_CH2));
		}
		break;
	default:
