	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap, AFE_ADDA_MTKAIF_RX_CFG0,
					   ...
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6895_DAI_ADDA);
		}
		break;
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mtkaif_dmic) {
			...
			regmap_update_bits(afe->regmap,
					   ...
					   MTKAIF_RXIF_VOICE_MODE_MASK_SFT,
					   0x0);
			mtk_adda_ul_src_dmic(afe, MT6895_DAI_ADDA_CH34);
		}
...
static bool is_adda_mtkaif_need_phase_delay(struct mt6895_afe_private *afe_priv)
{
...
/* mtkaif dmic */
static const char *const mt6895_adda_off_on_str[] = {
	"Off", "On"
};

static const struct soc_enum mt6895_adda_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt6895_adda_off_on_str),
			    mt6895_adda_off_on_str),
};

static int mt6895_adda_dmic_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6895_adda_dmic_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6895_adda6_only_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static int mt6895_adda6_only_set(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_kcontrol_chip(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mtk_adda_controls[] = {
	...
	SOC_ENUM_EXT("MTKAIF_DMIC", mt6895_adda_enum[0],
		     mt6895_adda_dmic_get, mt6895_adda_dmic_set),
	SOC_ENUM_EXT("MTKAIF_ADDA6_ONLY", mt6895_adda_enum[0],
		     mt6895_adda6_only_get, mt6895_adda6_only_set),
};
...
int mt6895_dai_adda_register(struct mtk_base_afe *afe)
{
	...
	/* set dai priv */
	ret = mt6895_dai_set_priv(afe, MT6895_DAI_ADDA,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	ret = mt6895_dai_set_priv(afe, MT6895_DAI_ADDA_CH34,
				  sizeof(struct mtk_afe_adda_priv), NULL);
	if (ret)
		return ret;

	/* ap dmic priv share with adda */
	afe_priv->dai_priv[MT6895_DAI_AP_DMIC] =
		afe_priv->dai_priv[MT6895_DAI_ADDA];
	afe_priv->dai_priv[MT6895_DAI_AP_DMIC_CH34] =
		afe_priv->dai_priv[MT6895_DAI_ADDA_CH34];
