...
/*
 * Card bind ordering helper: the machine driver defers its probe until
 * this returns true, so "Headset Jack" is always registered with the
 * card instead of being silently skipped.
 */
bool mt6368_accdet_ready(void)
{
	return !!accdet;
}
EXPORT_SYMBOL_GPL(mt6368_accdet_ready);
...
static struct accdet_priv MT6368_accdet[] = {
	{
		.caps = 0,
	},
};
...
static void mini_dump_register(void)
{
	...
	end_addr = RG_ACCDETSPARE_H_ADDR;
...
int mt6368_accdet_read_audio_res(unsigned int res_value)
{
...
EXPORT_SYMBOL(mt6368_accdet_read_audio_res);
...
static void send_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_MIC:
		 ...
		 */
		timer_delete_sync(&micbias_timer);
		break;
...
static inline void clear_accdet_int(void)
{
	...
	accdet_update_bit(ACCDET_IRQ_CLR_ADDR, ACCDET_IRQ_CLR_SFT);
}
...
static inline void clear_accdet_int_check(void)
{
	...
	accdet_clear_bit(ACCDET_IRQ_CLR_ADDR, ACCDET_IRQ_CLR_SFT);
...
static inline void clear_accdet_eint(u32 eintid)
{
	if ((eintid & PMIC_EINT0) == PMIC_EINT0) {
		accdet_update_bit(ACCDET_EINT0_IRQ_CLR_ADDR,
			ACCDET_EINT0_IRQ_CLR_SFT);
	}
	if ((eintid & PMIC_EINT1) == PMIC_EINT1) {
		accdet_update_bit(ACCDET_EINT1_IRQ_CLR_ADDR,
...
static inline void clear_accdet_eint_check(u32 eintid)
{
	...
	if ((eintid & PMIC_EINT0) == PMIC_EINT0) {
		while ((accdet_read(ACCDET_EINT0_IRQ_ADDR) & ACCDET_EINT0_IRQ_MASK_SFT)
			&& (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT)))
			;
		accdet_clear_bit(ACCDET_EINT0_IRQ_CLR_ADDR,
				ACCDET_EINT0_IRQ_CLR_SFT);
		accdet_update_bit(RG_INT_STATUS_ACCDET_ADDR,
				RG_INT_STATUS_ACCDET_EINT0_SFT);
	}
	if ((eintid & PMIC_EINT1) == PMIC_EINT1) {
		while ((accdet_read(ACCDET_EINT1_IRQ_ADDR) & ACCDET_EINT1_IRQ_MASK_SFT)
			&& (accdet_timeout_ns(cur_time, ACCDET_TIME_OUT)))
			;
		accdet_clear_bit(ACCDET_EINT1_IRQ_CLR_ADDR,
				ACCDET_EINT1_IRQ_CLR_SFT);
		accdet_update_bit(RG_INT_STATUS_ACCDET_ADDR,
...
static u32 adjust_eint_analog_setting(void)
{
	if ((accdet_dts.eint_detect_mode == 0x3) ||
		...
		/* ESD switches off */
		accdet_clear_bit(RG_ACCDETSPARE_H_ADDR, 0);
	}
...
static u32 adjust_eint_digital_setting(void)
{
	...
	if ((accdet_dts.eint_detect_mode == 0x1) ||
		...
		accdet_clear_bit(ACCDET_EINT0_CEN_STABLE_ADDR,
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		if (HAS_CAP(accdet->data->caps,
			...
			accdet_clear_bit(ACCDET_EINT0_CEN_STABLE_ADDR,
				...
				ACCDET_PMIC_EINT1)) {
			/* set DA stable signal */
			accdet_clear_bit(ACCDET_EINT1_CEN_STABLE_ADDR,
...
static void recover_eint_digital_setting(void)
{
	...
	if (accdet_dts.eint_detect_mode == 0x4) {
		...
		if (HAS_CAP(accdet->data->caps,
				ACCDET_PMIC_EINT0)) {
			/* enable eint0cen */
			accdet_update_bit(ACCDET_EINT0_CEN_STABLE_ADDR,
				ACCDET_EINT0_CEN_STABLE_SFT);
		} else if (HAS_CAP(accdet->data->caps,
				ACCDET_PMIC_EINT1)) {
			/* enable eint1cen */
			accdet_update_bit(ACCDET_EINT1_CEN_STABLE_ADDR,
				ACCDET_EINT1_CEN_STABLE_SFT);
		} else if (HAS_CAP(accdet->data->caps,
				ACCDET_PMIC_BI_EINT)) {
			/* enable eint0cen */
...
static u32 get_triggered_eint(void)
{
	...
	u32 irq_status = accdet_read(ACCDET_EINT0_IRQ_ADDR);
...
void accdet_set_debounce(int state, unsigned int debounce)
{
	...
	case accdet_state000:
		...
		accdet_write(ACCDET_DEBOUNCE0_L_ADDR, debounce & 0xff);
		accdet_write(ACCDET_DEBOUNCE0_H_ADDR, debounce >> 8);
		break;
	case accdet_state001:
		accdet_write(ACCDET_DEBOUNCE1_L_ADDR, debounce & 0xff);
		accdet_write(ACCDET_DEBOUNCE1_H_ADDR, debounce >> 8);
		break;
	case accdet_state010:
		accdet_write(ACCDET_DEBOUNCE2_L_ADDR, debounce & 0xff);
		accdet_write(ACCDET_DEBOUNCE2_H_ADDR, debounce >> 8);
		break;
	case accdet_state011:
		accdet_write(ACCDET_DEBOUNCE3_L_ADDR, debounce & 0xff);
		accdet_write(ACCDET_DEBOUNCE3_H_ADDR, debounce >> 8);
		break;
	case accdet_auxadc:
		/* set auxadc debounce:0x42(2ms) */
		accdet_write(ACCDET_CONNECT_AUXADC_TIME_DIG_L_ADDR,
			debounce & 0xff);
		accdet_write(ACCDET_CONNECT_AUXADC_TIME_DIG_H_ADDR,
			debounce >> 8);
		break;
	case eint_state000:
...
static u32 config_moisture_detect_2_1(void)
{
	...
	/* set moisture reference voltage MVTH */
	accdet_update_bits(RG_ACCDETSPARE_L_ADDR, 0x3, 0x1F, eintvth);
	return 0;
}
...
static int accdet_get_dts_data(void)
{
	...
	ret = of_property_read_u32(node,
			"accdet-eint-use_ext_res", &accdet_dts.eint_int_res_sel);
	if (ret)
		accdet_dts.eint_int_res_sel = 0;

	ret = of_property_read_u32(node,
			"accdet-eint-comp-vth", &accdet_dts.eint_comp_vth);
	if (ret)
		accdet_dts.eint_comp_vth = 2;
...
static void config_digital_init_by_mode(void)
{
	...
	if (accdet_dts.moisture_detect_mode == 0x1) {
		...
		accdet_clear_bit(ACCDET_EINT_M_DETECT_EN_ADDR,
			ACCDET_EINT_M_DETECT_EN_SFT);
		/* wk1, disable hwmode */
		accdet_write(ACCDET_EINT_HWMODE_EN_ADDR, 0x1);
	} else if (accdet_dts.moisture_detect_mode == 0x3) {
		/* disable moisture detection function */
		accdet_clear_bit(ACCDET_EINT_M_DETECT_EN_ADDR,
			ACCDET_EINT_M_DETECT_EN_SFT);
...
static void config_eint_init_by_mode(void)
{
	...
	if (accdet_dts.eint_detect_mode != 0x1) {
		...
		accdet_update_bits(RG_ACCDETSPARE_L_ADDR,
			RG_ACCDETSPARE_L_SFT,
			0x3, 0x3);
	}
...
static void accdet_init_once(void)
{
	...
	accdet_clear_bits(RG_AUDMICBIAS1HVEN_ADDR,
	...
	accdet_write(ACCDET_PWM_WIDTH_L_ADDR,
		REGISTER_VAL(cust_pwm_deb->pwm_width) & 0xff);
	accdet_write(ACCDET_PWM_WIDTH_H_ADDR,
		REGISTER_VAL(cust_pwm_deb->pwm_width) >> 8);
	accdet_write(ACCDET_PWM_THRESH_L_ADDR,
		REGISTER_VAL(cust_pwm_deb->pwm_thresh) & 0xff);
	accdet_write(ACCDET_PWM_THRESH_H_ADDR,
		REGISTER_VAL(cust_pwm_deb->pwm_thresh) >> 8);
	accdet_write(ACCDET_RISE_DELAY_L_ADDR, cust_pwm_deb->rise_delay);
	accdet_write(ACCDET_FALL_DELAY_ADDR,
		    cust_pwm_deb->fall_delay << 7 |
		    cust_pwm_deb->rise_delay >> 8);

	/* config micbias voltage, micbias1 vref is only controlled by accdet
	 * if we need 2.8V, config [12:13]
	 */
	if (accdet_dts.mic_vol <= 7) {
		reg = accdet_read(RG_AUDMICBIAS1VREF_ADDR);
		/* micbias1 <= 2.7V */
		accdet_write(RG_AUDMICBIAS1VREF_ADDR,
		reg | (accdet_dts.mic_vol<<RG_AUDMICBIAS1VREF_SFT) |
		RG_AUDMICBIAS1LOWPEN_MASK_SFT);
	} else if (accdet_dts.mic_vol == 8) {
		accdet_update_bit(RG_AUDMICBIAS1LOWPEN_ADDR,
				RG_AUDMICBIAS1LOWPEN_SFT);
		reg = accdet_read(RG_AUDMICBIAS1HVEN_ADDR);
		/* micbias1 = 2.8v */
		accdet_write(RG_AUDMICBIAS1HVEN_ADDR,
	...
	if (accdet_dts.mic_mode == HEADSET_MODE_1) {
		...
		accdet_update_bit(RG_ANALOGFDEN_ADDR,
			RG_ANALOGFDEN_SFT);
		accdet_update_bits(RG_ACCDETSPARE_H_ADDR, 3, 0x3, 0x3);
		...
		accdet_update_bit(RG_AUDMICBIAS1DCSW1PEN_ADDR,
...
void mt6368_accdet_late_init(unsigned long data)
{
	pr_info("%s()  now init accdet!\n", __func__);
	if (atomic_cmpxchg(&accdet_first, 1, 0)) {
		timer_delete_sync(&accdet_init_timer);
		accdet_init();
		accdet_init_debounce();
		accdet_init_once();
	} else
...
void mt6368_accdet_modify_vref_volt(void)
{
}
EXPORT_SYMBOL(mt6368_accdet_modify_vref_volt);
...
int mt6368_accdet_init(struct snd_soc_component *component,
			struct snd_soc_card *card)
{
	int ret;

	/*
	 * Card bind must not depend on this driver: if accdet failed to
	 * probe (e.g. auxadc not ready), its global state is dead. Skip
	 * jack creation and let the card live without jack detection.
	 */
	if (!accdet) {
		pr_warn("mt6368-accdet: driver not bound, skip jack init\n");
		return 0;
	}
...
static void accdet_remove(struct platform_device *pdev)
{
	destroy_workqueue(accdet->eint_workqueue);
	destroy_workqueue(accdet->dis_micbias_workqueue);
	destroy_workqueue(accdet->accdet_workqueue);
...
/* Module information */
MODULE_DESCRIPTION("MT6368 ALSA SoC accdet driver");
