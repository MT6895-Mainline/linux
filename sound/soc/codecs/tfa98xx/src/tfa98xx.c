...
/*
 * On downstream these forward to the MTK ADSP via audio_ipi; when the ADSP is
 * absent or in standby mtk-dsp-common.c returns -1 and playback proceeds with
 * the on-amp CoolDSP only. Replicate that behaviour without the ADSP stack.
 */
static int mtk_spk_send_ipi_buf_to_dsp(void *data_buffer, uint32_t data_size)
{
return -1;
}

static int mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size,
 uint32_t *buf_len)
{
return -1;
}
	...
	size_t count, loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) {
		mutex_lock(&tfa98xx->dsp_lock);
		error = tfa98xx_send_data_to_dsp(
			msg_file->data, msg_file->size);
	...
	struct snd_ctl_elem_info *uinfo)
{
	...
	pr_info("debug tfa98xx_info_profile %d\n",err);
	if (err != 0)
		return -EINVAL;
...
static atomic_t g_pwdn_mute;

static const char *tfa987x_pwdn_mute_text[] = {
	"DISABLED", "ENABLED"
};

static const struct soc_enum tfa987x_pwdn_mute_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(tfa987x_pwdn_mute_text),tfa987x_pwdn_mute_text)
};

static int tfa98xx_pwdn_mute(int mute)
{
	struct tfa98xx *tfa98xx = NULL;
	int ret = 0;
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
...
static int tfa987x_pwdn_set_mute(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s:	mute=%ld\n", __func__, ucontrol->value.integer.value[0]);

	atomic_set(&g_pwdn_mute, ucontrol->value.integer.value[0]);
	return tfa98xx_pwdn_mute(ucontrol->value.integer.value[0]);
}

static int tfa987x_pwdn_get_mute(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = atomic_read(&g_pwdn_mute);
	return 0;
}

const struct snd_kcontrol_new tfa987x_pwdn_controls[] = {
	SOC_ENUM_EXT("TFA987X_PWDN_MUTE", tfa987x_pwdn_mute_enum[0], tfa987x_pwdn_get_mute, tfa987x_pwdn_set_mute)
};
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
...
#endif
				if (!name){
				pr_debug("debug log yl:fail to malloc name\n");
					return -ENOMEM;
				}
...
#endif
	if (!name){
		pr_debug("debug log yl:fail to malloc name 2\n");
		return -ENOMEM;
	}
	...
	pr_debug("create tfa98xx_controls  ret=%d %num", ret,mix_index);
...
static const struct bin_attribute dev_attr_rw = {
	.attr = {
		.name = "rw",
...
static const struct bin_attribute dev_attr_reg = {
	.attr = {
		.name = "reg",
		.mode = S_IWUSR,
	},
											...
											size_t count, loff_t *ppos)
{
	...
	ret = tfa98xx_receive_data_from_dsp(
		buffer, count, &DataLength);
...
static int tfa98xx_i2c_probe(struct i2c_client *i2c)
{
    ...
    if ((no_start == 0) && (no_reset == 0)) {
		while (retry < PROBE_RETRY_TIMES) {
			ret = regmap_read(tfa98xx->regmap, 0x03, &reg);
			if (ret < 0) {
				dev_err(&i2c->dev, "Failed to read Revision register: %d, retry \n",
					ret);
	...
	tfa98xx_cache = kmem_cache_create("tfa98xx_cache", /* Cache name /proc/slabinfo */
		...
		(SLAB_HWCACHE_ALIGN | SLAB_RECLAIM_ACCOUNT |
			SLAB_RECLAIM_ACCOUNT), /* Cache property */
...
static int __init tfa98xx_i2c_init(void)
{
	...
	printk(KERN_ERR"TFA98XX driver version %s\n", TFA98XX_VERSION);
