...
/*
 * ATF high-temp-aging variant flag (the vendor consults ATF via SIPC;
 * mainline keeps a module param). Default: normal trip point.
 */
...
{
	struct soc_temp_tz *lvts_tz = thermal_zone_device_priv(tz);
...
static void lvts_wait_for_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	...
	while (cnt < 50) {
		...
		if ((cnt + 1) % 10 == 0) {
					...
					cnt + 1, (unsigned int)(temp >> 16),
					((unsigned int)(temp & BIT(2)) >> 2),
					((unsigned int)(temp & BIT(1)) >> 1),
					(unsigned int)(temp & BIT(0)),
					(unsigned int)(temp & GENMASK(10,0)));
		}
...
static void set_all_tc_hw_reboot(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		/*
		 * If the board is a high-temp-aging ATF variant, force trip
		 * temp = 200'C. The vendor consults ATF via SIPC; mainline
		 * keeps a module param (default: normal trip point).
		 */
		if (!high_temp_aging) {
	...
	struct platform_device *pdev)
{
	...
	for (i = 0; i < lvts_data->num_domain; i++) {
		...
		/* Get interrupt number (platform_get_irq handles of_irq
		 * translation even when IORESOURCE_IRQ entries are absent) */
		{
			int irq = platform_get_irq(pdev, i);
			if (irq < 0) {
				dev_err(dev, "No irq resource, index %d (%d)\n", i, irq);
				return irq;
			}
