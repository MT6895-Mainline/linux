...
/*
 * pearl boot-stage marker writer (XAGR ring), built into the kernel.
 *
 * pearl (Redmi Note 11T Pro / POCO X4 GT / Redmi K50i, MT6895) boot trace.
 ...
 * never confirmed loaded): CONFIG_PEARL_MARKER_WRITER is set only by the pearl
...
#define PEARL_MRDUMP_PA	0x7ffbf000UL
#define PEARL_MRDUMP_SZ	0x10000UL
#define PEARL_RING_OFF	0x2000U
#define PEARL_RING_SZ	0xE000U
#define PEARL_MAGIC	0x52474158UL	/* "XAGR" */
#define PEARL_MAX_MSG	256

static void __iomem *pearl_mr_base;
static bool pearl_early_map;	/* still using the early_ioremap fixmap slot */
static bool pearl_direct_map;	/* using the direct map (needs cache clean) */

static void pearl_marker_ring_write(const char *buf, int n)
{
	void __iomem *ring;
	u32 cursor;
	int i;
	...
	writel(PEARL_MAGIC, pearl_mr_base + 0x0000);
	cursor = readl(pearl_mr_base + 0x0004);
	ring = pearl_mr_base + PEARL_RING_OFF;
	for (i = 0; i < n; i++)
		writeb(buf[i], ring + ((cursor + i) % PEARL_RING_SZ));
	writel(cursor + n, pearl_mr_base + 0x0004);
	writel(readl(pearl_mr_base + 0x0008) + n, pearl_mr_base + 0x0008);
	/*
	 ...
	 */
	if (pearl_direct_map)
		dcache_clean_poc((unsigned long)pearl_mr_base,
				 (unsigned long)pearl_mr_base + PEARL_MRDUMP_SZ);
}

void pearl_marker_put(const char *fmt, ...)
{
	va_list args;
	char buf[PEARL_MAX_MSG];
	int n;
	...
	if (n <= 0)
		return;
	pearl_marker_ring_write(buf, n);
}
EXPORT_SYMBOL_GPL(pearl_marker_put);

void pearl_marker_stage(u32 stage)
{
	if (!pearl_mr_base)
		return;
	writel(stage, pearl_mr_base + 0x1000);
	pearl_marker_put("stage=%u\n", stage);
}
EXPORT_SYMBOL_GPL(pearl_marker_stage);
...
static unsigned int pearl_mirror_cnt;
...
void pearl_marker_early_printk(const char *fmt, va_list args)
{
	...
	if (n <= 0)
		return;
	pearl_marker_ring_write(buf, n);
	if (++pearl_mirror_cnt % 64 == 0) {
		char hb[32];
		int hn = snprintf(hb, sizeof(hb), "MIRROR:%u\n", pearl_mirror_cnt);

		pearl_marker_ring_write(hb, hn);
	}
}
...
void __init pearl_marker_early_init(void)
{
	pearl_mr_base = early_ioremap(PEARL_MRDUMP_PA, PEARL_MRDUMP_SZ);
	if (!pearl_mr_base) {
		pr_info("pearl-marker-writer: early_ioremap 0x%08lx failed\n",
			PEARL_MRDUMP_PA);
		return;
	}
	pearl_early_map = true;
	/* fresh ring per boot: only the last boot's markers survive */
	writel(PEARL_MAGIC, pearl_mr_base + 0x0000);
	writel(0, pearl_mr_base + 0x0004);
	writel(0, pearl_mr_base + 0x0008);
	writel(0, pearl_mr_base + 0x1000);
	pr_info("pearl-marker-writer: XAGR ring armed at 0x%08lx\n",
		PEARL_MRDUMP_PA);
	pearl_marker_stage(1);
}

static int pearl_marker_module_nb(struct notifier_block *nb,
...
static struct notifier_block pearl_marker_nb = {
	.notifier_call = pearl_marker_module_nb,
};

static int __init pearl_marker_w_late_init(void)
{
	void *perm;
	 ...
	 */
	if (pearl_early_map) {
		perm = memremap(PEARL_MRDUMP_PA, PEARL_MRDUMP_SZ, MEMREMAP_WB);
		if (perm) {
			pearl_mr_base = perm;
			pearl_early_map = false;
			pearl_direct_map = true;
			dcache_clean_poc((unsigned long)pearl_mr_base,
					 (unsigned long)pearl_mr_base +
					 PEARL_MRDUMP_SZ);
			pearl_marker_put("marker writer: permanent mapping armed\n");
		}
	}
	pearl_marker_put("marker writer built-in init\n");
	register_module_notifier(&pearl_marker_nb);
	return 0;
}
core_initcall(pearl_marker_w_late_init);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("pearl boot-stage marker writer (XAGR ring), built-in");
