...
/*
 * qqcandy: the vendor file pulled in the AEE exception framework, the
 * mrdump crash-dump stack and the MTK boot/mboot parameter helpers. None of
 * those exist in this tree, so the include sites are dropped here together
 * with every call into them (each removal is marked with a qqcandy comment).
 */
...
/*
 * qqcandy: report-only hang detector, gated off by default. With the
 * parameter false, monitor_hang_init() returns early: no misc device, no
 * kthreads, no wait queues, nothing. Enable it from the kernel command line
 * (monitor_hang.monitor_hang_enable=1) because the init-time gate also means
 * a sysfs write after boot is too late:
 *
 *   /sys/module/monitor_hang/parameters/monitor_hang_enable
 */
static bool monitor_hang_enable;
module_param(monitor_hang_enable, bool, 0644);
...
{
	/*
	 * qqcandy: report-only. The vendor body called aee_rr_*() and
	 * mrdump_regist_hang_bt()/mrdump_common_die(), or panic() when
	 * AEE_IPANIC was off. None of that infrastructure exists in this tree
	 * and this detector must never panic, so it only logs. This entire
	 * block is additionally not selectable (no CONFIG_MTK_HANG_DETECT_DB
	 * in Kconfig).
	 */
	pr_notice("[Hang_Detect] hang DB dump requested (report-only)\n");
}
#endif
...
void show_thread_info(struct task_struct *p, bool dump_bt)
{
	...
	/* qqcandy: task_struct::cpu is gone in 6.18; use task_cpu(). */
	log_hang_info("%d ", task_cpu(p));
	hang_log("%d ", task_cpu(p));
#endif
...
#ifdef CONFIG_STACKTRACE
	/* qqcandy: task_struct::state was renamed __state in 6.18. */
	if (dump_bt || ((!p->mm || READ_ONCE(p->__state) == TASK_RUNNING ||
			READ_ONCE(p->__state) & TASK_UNINTERRUPTIBLE) &&
			!strstr(p->comm, "wdtk")))
...
static int dump_native_maps(pid_t pid, struct task_struct *current_task)
{
	...
	/* qqcandy: 6.18 removed mm->mmap / vm_next; iterate the maple tree. */
	VMA_ITERATOR(vmi, current_mm, 0);
	...
	struct task_struct *current_task)
{
	...
	if (compat_user_mode(user_ret)) {	/* K64_U32 for check reg */
		...
		/*
		 * qqcandy: mm->mmap/vm_next are gone in 6.18. find_vma()
		 * returns the first VMA ending after the address; require it
		 * to start at or below SP so a gap is not taken for a stack.
		 */
		vma = find_vma(current_mm, userstack_start);
		if (vma && vma->vm_start <= userstack_start)
			userstack_end = vma->vm_end;
		...
		/* qqcandy: mm->mmap/vm_next -> find_vma(), as above. */
		vma = find_vma(current_mm, userstack_start);
		if (vma && vma->vm_start <= userstack_start)
			userstack_end = vma->vm_end;
...
/*
 * qqcandy: vendor registered this as the mrdump all-task callback
 * (mrdump_regist_hang_bt), which no longer exists here. Keep the helper for
 * reference but mark it unused so it does not warn.
 */
...
static void show_status(int flag)
{
	...
	if (Hang_first_done)	{ /* the last dump */
		...
		/*
		 * qqcandy: vendor called show_free_areas(0, NULL) here. In
		 * 6.18 that helper is static inside mm/show_mem.c with an
		 * extra max_zone_idx argument and is not callable from a
		 * driver, so the memory-utilisation dump is dropped.
		 */
		run_callback();
...
static int hang_detect_thread(void *arg)
{
	...
	while (1) {
		...
		/* so create new thread to dump enough kernel bt */
				...
				if (Hang_first_done == true) {
					/*
					 * qqcandy: vendor called trigger_hang_db()
					 * (mrdump + AEE) here, or BUG() when that
					 * config was off. Neither infrastructure
					 * exists in this tree and this detector
					 * must never turn a hang into a reboot, so
					 * report the confirmed hang and stop.
					 */
					pr_notice("[Hang_Detect] hang confirmed, report-only; detection stopped\n");
					hd_detect_enabled = false;
				} else {
...
static int __init monitor_hang_init(void)
{
	...
	/*
	 * qqcandy: default-off gate. Returning before any allocation,
	 * registration or kthread creation leaves the system completely
	 * untouched unless the feature is deliberately enabled.
	 */
	if (!monitor_hang_enable) {
		pr_info("monitor_hang: gated off (monitor_hang_enable=0)\n");
		return 0;
	}
	...
	hang_detect_init();
	/* qqcandy: no mrdump here; vendor registered show_task_info with it. */
