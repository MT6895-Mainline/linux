	...
	struct cmdq_thread *thread, struct list_head *removes)
{
	...
	if (irq_flag & CMDQ_THR_IRQ_ERROR) {
		...
		pr_err("PEARL-GCE: thr%d EN=0x%x STATUS=0x%08x PC=0x%08x IRQ=0x%08x\n",
			...
			readl(thread->base + CMDQ_THR_ENABLE_TASK),
			readl(thread->base + CMDQ_THR_CURR_STATUS),
			cmdq_thread_get_pc(thread), irq_flag);
...
{
	struct cmdq_thread *thread = container_of(t, struct cmdq_thread, timeout);
	struct cmdq *cmdq = container_of(thread->chan->mbox, struct cmdq, mbox);
	unsigned long flags;
	bool empty;
	...
	u64 **inst_out, dma_addr_t *pc_out)
{
	...
	gctl = readl(cmdq->base + GCE_GCTL_VALUE);
	...
	cmdq_util_user_msg(chan,
		"rst:%#x en:%#x suspend:%#x status:%#x irq:%x en:%#x cfg:%#x gctl:%#x",
		warn_rst, en, suspend, status, irq, irq_en, cfg, gctl);
