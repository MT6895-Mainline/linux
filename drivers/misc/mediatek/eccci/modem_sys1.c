...
/* ===== XAGA-MDLOG-BEGIN (generated by patch-mdlog.py) ===== */
/* XAGA: MODEM 日志使能（mdlogger / TST 握手）用的最小 CCCI 发送点。
 *
 ...
 */
/* XAGA-MDLOG: 自动首发开关。默认 0（关闭），要把 60 条 0x0C 自动塞进
 * 基带启动窗口时，由用户态先 arm：
...
static unsigned int xaga_mdlog_last_msg;
static unsigned int xaga_mdlog_last_resv;
static int xaga_mdlog_last_ret = 0x7fffffff;
...
static ssize_t md_cd_mdlog_show(struct ccci_modem *md, char *buf)
{
	return snprintf(buf, 320,
		...
		xaga_mdlog_last_msg, xaga_mdlog_last_resv,
		xaga_mdlog_last_ret, xaga_mdlog_last_state,
		xaga_mdlog_send_cnt);
}
	...
	const char *buf, size_t count)
{
	...
	if (n < 1) {
		CCCI_ERROR_LOG(md->index, TAG,
			"XAGA-MDLOG: bad input (need 'msg [resv] [blocking]')\n");
		return -EINVAL;
	}
	xaga_mdlog_send(msg, resv, blocking, "sysfs");
	return count;
}

/* 只有在 xaga_mdlog_auto 打开时才发（给 AMMS 之类的自动触发点用） */
int xaga_mdlog_send_armed(unsigned int msg, unsigned int resv, int blocking,
	const char *why)
{
	if (!xaga_mdlog_auto)
		return -1;
	return xaga_mdlog_send(msg, resv, blocking, why);
}

/* XAGA-MDLOG: 自动首发（受 xaga_mdlog_auto 开关约束，默认关闭）。
 *
