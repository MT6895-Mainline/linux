...
int reset_scp(int reset)
{
	...
	/* request pll clock before turn on scp */
	if (scp_dvfs_feature_enable()) {
		ret = scp_pll_ctrl_set(PLL_ENABLE, CLK_26M);
		if (ret)
			return ret;
	}
#endif
...
void scp_region_info_init(void)
{
	...
	pr_notice("[SCP] LK region: loader=%08x/%x firmware=%08x/%x dram=%08x/%x struct=%x\n",
