...
void wait_scp_dvfs_init_done(void)
{
	...
	/*
	 * Bound the vendor init wait when the DVFS node is absent or its
	 * probe fails. Device initcalls execute after SMP bring-up.
	 */
	if (!g_scp_dvfs_enable)
		return;
	...
	while (g_scp_dvfs_init_flag != 1) {
		...
		count++;
		if (count > 3000) {
			pr_notice("SCP dvfs driver init fail, continue without dvfs\n");
			return;
		}
	}
}
