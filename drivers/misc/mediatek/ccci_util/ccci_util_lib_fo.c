...
/* qqcandy: vunmap() needs vmalloc.h; the vendor tree got it transitively */
#include <linux/vmalloc.h>
/* qqcandy: memremap()/MEMREMAP_WB for the bounded runtime tag read */
#include <linux/io.h>
...
/*
 * qqcandy: master gate for the LK/modem tag parse.
 *
 * collect_lk_boot_arguments() -> find_ccci_tag_inf() -> nc_smem_info_parsing()
 * reads the LK tag blob through ioremap/vmap mappings (memcpy_fromio), and
 * nc_smem_info_parsing()/dump_retrieve_info() additionally call
 * free_reserved_memory() on AP/MD shared regions. None of that may happen on a
 * normal boot, so the whole path is off unless the gate is explicitly set.
 *
 * qqcandy: the gate is now a plain built-in bool that is always false (see
 * ccci_util_lib_main.c); the tunable runtime trigger moved out with the bounded
 * parse into the separate ccci_probe.ko module (MTK_CCCI_PROBE_MODULE). This
 * built-in path therefore can never be armed, and the safe bounded parse is no
 * longer part of the boot image at all.
 */
/*************************************************************************
