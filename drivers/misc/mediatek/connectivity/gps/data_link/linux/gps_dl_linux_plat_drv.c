...
static int gps_dl_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < GPS_DL_IRQ_NUM; i++) {
		/* PEARL: 主线不再为 DT 设备填 IORESOURCE_IRQ 资源，
		 * platform_get_resource(IORESOURCE_IRQ) 恒为 NULL，改用 platform_get_irq()。
		 */
		int irq_id = platform_get_irq(pdev, i);

		if (irq_id < 0) {
			GDL_LOGE_INI("irq idx = %d, platform_get_irq fail %d", i, irq_id);
			continue;
		}

		GDL_LOGW_INI("irq idx = %d, id = %d", i, irq_id);
		gps_dl_irq_set_id(i, irq_id);
	}
