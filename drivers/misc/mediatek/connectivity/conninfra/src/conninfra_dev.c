...
static int conninfra_thermal_get_temp_cb(struct thermal_zone_device *tz, int *temp);
...
/*
 * Use late_initcall instead of module_init: on mainline the MT6895 SCP/SPM
 * power-domain provider (scpsys) may not have probed yet during the normal
 * device_initcall phase.  The downstream consys_hw_init() waits synchronously
