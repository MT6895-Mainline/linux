...
static int is_skb_gro(struct sk_buff *skb)
{
	...
	if (protocol == IPPROTO_TCP) {
		...
		if (g_cur_dl_speed > 300000000LL) //>300M
			return 1;
	}
...
static void ccmni_napi_poll_timeout(struct timer_list *t)
{
	...
	//struct ccmni_instance *ccmni = timer_container_of(ccmni, t, timer);
	...
	struct net_device *dev)
{
	...
	if (dev && (ctlb->ccci_ops->md_ability & MODEM_CAP_NAPI)) {
		...
		if (ctlb->ccci_ops->napi_poll_weigh == NAPI_POLL_WEIGHT)
...
#if 1 /* qqcandy: ccmni is part of ccci_md_all.ko */
