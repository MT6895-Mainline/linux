...
/*
 * qqcandy: default-off gate. masp_probe() returns -ENODEV unless this is
 * set, so the HACC registers are never touched, /dev/sec is not created and
 * no security state is read or written on a normal boot. Enable it
 * deliberately with
 *
 *   echo 1 > /sys/module/sec/parameters/masp_enable
 *   echo 1c009000.masp > /sys/bus/platform/drivers/masp/bind
 *
 * or "sec.masp_enable=1" on the kernel command line (KBUILD_MODNAME is sec).
 */
static bool masp_enable;
module_param(masp_enable, bool, 0644);
...
static int sec_init(struct platform_device *dev)
{
	...
	/*
	 * qqcandy: 6.18 class_create() no longer takes the module owner.
	 */
...
int masp_probe(struct platform_device *dev)
{
	...
	if (!masp_enable) {
		pr_notice("[%s] masp probe gated off (masp_enable=0)\n",
			  SEC_DEV_NAME);
		return -ENODEV;
	}
