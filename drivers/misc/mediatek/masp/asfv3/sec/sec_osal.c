 ...
 *****************************************************************************/
/*
 * qqcandy: 6.18 DEFINE_SEMAPHORE() takes an initial count; the vendor
 * one-argument form no longer exists. These are all plain binary
 * semaphores, so the count is 1.
 */
DEFINE_SEMAPHORE(hacc_sem, 1);
...
/*
 * qqcandy: the vendor osal_set_kernel_fs()/osal_restore_fs() wrapped
 * get_fs()/set_fs(KERNEL_DS). mm_segment_t and set_fs() were removed from
 * the kernel, and no code in this self-contained port uses ASF_GET_DS /
 * ASF_PUT_DS, so both helpers (and the sec_mm_sem they took) are dropped.
 */
