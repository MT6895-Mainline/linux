...
/* IOMMU ATF secure-bank helpers (downstream-compatible). */
#define SMC_IOMMU_SUCCESS		(0)
#define SMC_IOMMU_FAIL			(1)
#define SMC_IOMMU_NONSUPPORT		(-1)

#define IOMMU_ATF_SET_CMD(type, id, bank, cmd) \
	((cmd) | ((bank) << 8) | ((id) << 16) | ((type) << 24))

enum mtk_iommu_secure_atf_cmd {
	SECURE_BANK_INIT,
	SECURE_BANK_IRQ_EN,
	SECURE_BANK_BACKUP,
	SECURE_BANK_RESTORE,
...
static int mtk_iommu_hw_is_valid(u32 type, u32 id, u32 bank)
{
...
static int mtk_iommu_atf_call(u32 type, u32 id, u32 bank,
			      unsigned long cmd, unsigned long in2,
			      ...
			      unsigned long in7)
{
	...
	arm_smccc_smc(MTK_SIP_KERNEL_IOMMU_CONTROL, cmd, in2, in3, in4,
		      in5, in6, in7, &res);

	return res.a0;
}

static int mtk_iommu_sec_bk_init_by_atf(u32 type, u32 id)
{
	unsigned long cmd = IOMMU_ATF_SET_CMD(type, id, IOMMU_BK4,
					      SECURE_BANK_INIT);
	int ret;

	ret = mtk_iommu_atf_call(type, id, IOMMU_BK4, cmd,
				 0, 0, 0, 0, 0, 0);
	if (ret) {
...
static int mtk_iommu_sec_bk_irq_en_by_atf(u32 type, u32 id, unsigned long en)
{
	unsigned long cmd = IOMMU_ATF_SET_CMD(type, id, IOMMU_BK4,
					      SECURE_BANK_IRQ_EN);
	int ret;
	...
	ret = mtk_iommu_atf_call(type, id, IOMMU_BK4, cmd, en,
				 0, 0, 0, 0, 0);
	if (ret) {
...
static int mtk_iommu_secure_bk_backup_by_atf(u32 type, u32 id)
{
unsigned long cmd = IOMMU_ATF_SET_CMD(type, id, IOMMU_BK4,
      SECURE_BANK_BACKUP);
int ret;
...
static int mtk_iommu_secure_bk_restore_by_atf(u32 type, u32 id)
{
unsigned long cmd = IOMMU_ATF_SET_CMD(type, id, IOMMU_BK4,
      SECURE_BANK_RESTORE);
int ret;
...
static int mtk_iommu_secure_bk_tf_dump(u32 type, u32 id, u32 bank,
...
static int ao_secure_dbg_switch_by_atf(u32 type, u32 id, unsigned long en)
{
return SMC_IOMMU_SUCCESS;
}

static void report_custom_iommu_fault(u64 iova, u64 pa, u32 fault_id,
      enum mtk_iommu_type type, u32 id)
{
pr_warn("IOMMU fault type=%u id=%u iova=0x%llx pa=0x%llx fault_id=0x%x\n",
type, id, iova, pa, fault_id);
}
...
{
struct mtk_iommu_data *data = container_of(t, struct mtk_iommu_data, iommu_isr_pause_timer);
...
/* Minimal stubs for unused PERI/APU debug paths in the MT6895 MM port. */
static enum peri_iommu get_peri_iommu_id(u32 bus_id)
{
	return PERI_IOMMU_M4;
}
			     ...
			     bool enable, unsigned int domid)
{
	...
	for (i = 0; i < fwspec->num_ids; ++i) {
		...
		pr_info("XAGA-IOMMU: config %s iommu_dev:%s user_dev:%s larb:%d port:%d dom:%d bank:%d mmu:0x%x->0x%x\n",
			...
			larb_mmu->mmu,
			enable ? (larb_mmu->mmu | MTK_SMI_MMU_EN(portid)) :
...
static void mtk_iommu_set_dev_dma(struct device *dev)
{
	if (!dev)
		return;
				   ...
				   struct device *dev)
{
	...
	if (!data->m4u_dom) { /* Initialize the M4U HW */
		 ...
		 */
		if (data->plat_data->iommu_id != MDP_IOMMU)
			pm_runtime_put(m4udev);
	}
...
return dom->iop->map_pages(dom->iop, iova, paddr, pgsize, pgcount,
  prot, gfp, mapped);
}
