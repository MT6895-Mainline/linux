 ...
 *
 * Ported from the MT6895 downstream kernel for mainline. Trimmed of the
 * Android-only TCPC suspend interaction (tcpc_dev_get_by_name / tcpm_*), and
 * the threaded IRQ request is guarded: the mt6895 pinctrl has no EINT support
 * in mainline, so the INT pin (GPIO 116) cannot be routed yet.
 */
...
static int mt6375_check_devid(struct mt6375_data *ddata)
{
	...
	if (vid != MT6375_VID) {
		dev_warn(ddata->dev, "vendor id 0x%02X != 0x%02X; continuing "
			"(bring-up)\n", vid, MT6375_VID);
	}
	ddata->chip_rev = val & MT6375_MSK_CHIP_REV;
	dev_info(ddata->dev, "MT6375 found (DEV_INFO 0x%02X, rev 0x%02X)\n",
		 val, ddata->chip_rev);
	return 0;
}
...
static int __maybe_unused mt6375_suspend(struct device *dev)
{
	...
	if (device_may_wakeup(dev) && i2c->irq > 0)
...
MODULE_DESCRIPTION("MT6375 Core I2C Driver");
