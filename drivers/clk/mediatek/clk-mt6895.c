...
/*
 * Downstream MT6895 uses the old MediaTek hardware-voter mux API. Mainline
 * does not provide MUX_HWV; the display/mminfra muxes work fine as normal
 * gated CLR/SET/UPD muxes for this bring-up, so discard the HWV arguments.
 */
#define MUX_HWV(_id, _name, _parents, _mux_ofs, _mux_set_ofs, _mux_clr_ofs, \
_hwv_sta_ofs, _hwv_set_ofs, _hwv_clr_ofs, _shift, _width, \
_gate, _upd_ofs, _upd)\
MUX_GATE_HWV_CLR_SET_UPD(_id, _name, _parents, _mux_ofs, _mux_set_ofs, \
_mux_clr_ofs, _hwv_sta_ofs, _hwv_set_ofs, _hwv_clr_ofs, \
...
/*
 * MT6895's MFGPLL/MFGSCPLL use a higher VCO floor than the generic
 * MT6895_PLL_FMIN. The generic 1500MHz floor makes mtk_pll_calc_values()
 * pick postdiv=2 for 852MHz (VCO=1704MHz), which does not lock. With the
 * downstream 750MHz-950MHz/posdiv4 range we need postdiv=4 (VCO=3408MHz),
 * so use a 2000MHz floor for the two GPU PLLs.
 */
#define MT6895_MFG_PLL_FMIN	(2000UL * MHZ)

#define PLL_MFG(_id, _name, _reg, _en_reg, _en_mask, _pll_en_bit,	\
			_pwr_reg, _flags, _rst_bar_mask,		\
			_pd_reg, _pd_shift, _tuner_reg,			\
			_tuner_en_reg, _tuner_en_bit,			\
			_pcw_reg, _pcw_shift, _pcwbits) {		\
...
static const struct mtk_pll_data mfg_ao_plls[] = {
	PLL_MFG(CLK_MFG_AO_MFGPLL, "mfg_ao_mfgpll", MFGPLL_CON0/*base*/,
		MFGPLL_CON0, 0, 0/*en*/,
...
static const struct mtk_pll_data mfgsc_ao_plls[] = {
	PLL_MFG(CLK_MFGSC_AO_MFGSCPLL, "mfgsc_ao_mfgscpll", MFGSCPLL_CON0/*base*/,
...
static void __maybe_unused mt6895_pll_force_off(void)
{
	int i;
