...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (is_tp_doubleclick_enable() == false ||get_panel_dead_flag()) {
		...
		pr_debug("is_tp_doubleclick_enable will true");
		/* DVDD Power off */
		ctx->dvdd_gpio = devm_gpiod_get_index(ctx->dev,
			"dvdd", 0, GPIOD_OUT_HIGH);
		if (IS_ERR(ctx->dvdd_gpio)) {
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	{
		struct gpio_desc *err_irq;

		err_irq = devm_gpiod_get(dev, "mi,esd-err-irq", GPIOD_IN);
		if (!IS_ERR(err_irq))
			ext_params.err_flag_irq_gpio = desc_to_gpio(err_irq);
	}
