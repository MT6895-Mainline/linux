				  ...
				  struct drm_atomic_state *atomic_state)
{
	...
	crtc_state = drm_atomic_get_crtc_state(atomic_state, state->crtc);
	...
	if (mtk_crtc->res_switch && (drm_crtc_index(state->crtc) == 0)) {
		...
		struct mtk_crtc_state *old_mtk_state = to_mtk_crtc_state(
			drm_atomic_get_old_crtc_state(atomic_state, state->crtc));
				    ...
				    struct drm_atomic_state *atomic_state)
{
	...
	dst_x = new_state->dst.x1;
	dst_y = new_state->dst.y1;
	...
	if (state->pending.mml_mode == MML_MODE_RACING && mtk_crtc->is_mml) {
		...
		state->pending.enable = new_state->visible;
		...
		state->pending.src_x = (new_state->src.x1 >> 16);
		state->pending.src_y = (new_state->src.y1 >> 16);
