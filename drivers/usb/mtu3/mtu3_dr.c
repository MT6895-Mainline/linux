...
static int switch_port_to_host(struct ssusb_mtk *ssusb)
{
	u32 check_clk = 0;
	int ret;
...
static void ssusb_mode_sw_work_locked(struct work_struct *work)
{
	...
	if (current_role == desired_role && !otg_sx->role_switch_failed)
		goto update_vbus;

	/* Publish success only after the entire hardware transition completes. */
	otg_sx->role_switch_failed = true;
	...
	case USB_ROLE_HOST:
		...
		mtu3_stop(mtu);
		ret = switch_port_to_host(ssusb);
		if (ret)
			goto out;
		ret = ssusb_set_vbus(otg_sx, 1);
		if (ret)
			goto out;
		ssusb->is_host = true;
		break;
	...
	otg_sx->role_switch_failed = false;
update_vbus:
