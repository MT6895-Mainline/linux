...
char *printHex_data(char *label, u8 *buff, int count)
{
	...
	offset = strlen(label);
	result = kmalloc(((offset + 4 * count) + 1) * sizeof(char),
			 GFP_KERNEL);
	if (result != NULL) {
		strscpy(result, label, offset);
...
int u8ToU16n(u8 *src, int src_length, u16 *dst)
{
	...
	j = 0;
	dst = kmalloc((src_length / 2) * sizeof(u16), GFP_KERNEL);
...
int u16ToU8n_be(u16 *src, int src_length, u8 *dst)
{
	...
	dst = kmalloc((2 * src_length) * sizeof(u8), GFP_KERNEL);
	j = 0;
...
short **array1dTo2d_short(short *data, int size, int columns)
{
	...
	int i;
	short **matrix = kmalloc(((int)(size / columns)) * sizeof(short *),
				 GFP_KERNEL);
	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++) {
			matrix[i] = kmalloc_array(columns, sizeof(short),
						  GFP_KERNEL);
		}
...
u16 **array1dTo2d_u16(u16 *data, int size, int columns)
{
	...
	int i;
	u16 **matrix = kmalloc(((int)(size / columns)) * sizeof(u16 *), GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(u16), GFP_KERNEL);
...
u8 **array1dTo2d_u8(u8 *data, int size, int columns)
{
	...
	int i;
	u8 **matrix = kmalloc(((int)(size / columns)) * sizeof(u8 *), GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(u8), GFP_KERNEL);
...
i8 **array1dTo2d_i8(i8 *data, int size, int columns)
{
	...
	int i;
	i8 **matrix = kmalloc(((int)(size / columns)) * sizeof(i8 *), GFP_KERNEL);

	if (matrix != NULL) {
		for (i = 0; i < (int)(size / columns); i++)
			matrix[i] = kmalloc_array(columns, sizeof(i8), GFP_KERNEL);
 ...
 * @param size the size of src (can be <= 8)
 * @return OK if success or ERROR_OP_NOT_ALLOW if size exceed 8
 */
