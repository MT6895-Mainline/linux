...
static struct gpio_desc *reset_gpio;	/*reset pin gpio descriptor, NULL if the reset pin is not connected*/
...
int writeLockDownInfo(u8 *data, int size, u8 lock_id)
{
	...
	for (i = 0; i < 3; i++) {
		...
		logError(0, "%s %s: Get the data CRC value:%02X\n", tag,
			 __func__, crc_data);
		ret = fts_writeU8UX(LOCKDOWN_WRITE_CMD, BITS_16,
...
int readLockDownInfo(u8 *lockData, u8 lock_id, int size)
{
	...
	temp = kmalloc(LOCKDOWN_LENGTH * sizeof(u8), GFP_KERNEL);
	if (temp == NULL) {
	...
	fts_disableInterrupt();
	for (i = 0; i < 3; i++) {
		ret = fts_writeReadU8UX(LOCKDOWN_READ_CMD, BITS_16,
...
int fts_get_lockdown_info(u8 *lockData, struct fts_ts_info *info)
{
	...
	temp = kmalloc(1024 * sizeof(u8), GFP_KERNEL);
	if (temp == NULL) {
		logError(1, "FTS temp alloc  memory failed\n");
		return -ENOMEM;
	}
