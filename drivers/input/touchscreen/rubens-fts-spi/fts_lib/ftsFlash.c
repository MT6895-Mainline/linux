...
int getFWdata(const char *pathToFile, u8 **data, int *size)
{
	...
	case 1:
		...
		*data = kmalloc((*size) * sizeof(u8), GFP_KERNEL);
		if (*data == NULL) {
			logError(1,
				 "%s %s: Impossible to allocate memory! ERROR %08X\n",
		...
		if (dev != NULL) {
			...
			if (res == 0) {
				...
				*data = kmalloc((*size) * sizeof(u8), GFP_KERNEL);
				...
				logError(1,
					 "%s %s: No File found! ERROR %08X\n",
					 tag, __func__, ERROR_FILE_NOT_FOUND);
				return ERROR_FILE_NOT_FOUND;
			}
		} else {
			logError(1,
				 "%s %s: No device found! ERROR %08X\n",
				 tag, __func__,
				 ERROR_OP_NOT_ALLOW);
...
int readFwFile(const char *path, Firmware *fw, int keep_cx)
{
	...
	if (res < OK) {
		logError(1,
			 "%s %s: impossible retrieve FW... ERROR %08X\n",
			 tag, __func__,
			 ERROR_MEMH_READ);
...
int parseBinFile(u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
	...
	if (fw_size < FW_HEADER_SIZE + FW_BYTES_ALLIGN || fw_data == NULL) {
		logError(1,
			 ...
			 tag, __func__, fw_size, FW_HEADER_SIZE + FW_BYTES_ALLIGN,
			 ERROR_FILE_PARSE);
		...
		if (dimension + FW_HEADER_SIZE + FW_BYTES_ALLIGN != temp) {
			logError(1,
				 "%s %s: Read only %d instead of %d... ERROR %08X\n",
				 tag, __func__, fw_size, dimension + FW_HEADER_SIZE +
				 FW_BYTES_ALLIGN, ERROR_FILE_PARSE);
			res = ERROR_FILE_PARSE;
			goto END;
		}

		fwData->data = kmalloc(dimension * sizeof(u8),
					     GFP_KERNEL);
		if (fwData->data == NULL) {
			logError(1, "%s %s: ERROR %08X\n", tag, __func__,
				 ERROR_ALLOC);
		...
		logError(0, "%s Code Pages: %d panel area Pages: %d cx area Pages: %d fw config Pages: %d !\n",
			 tag,
		...
		logError(0, "%s Code start addr: 0x%08X cx start addr: 0x%08X fw start addr: 0x%08X !\n",
			 tag,
...
int flash_erase_page_by_page(ErasePage keep_cx, Firmware *fw)
{
	...
	u8 cmd[6] = { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_CODE0, 0x80 };
	u8 cmd2[11] = { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x01, 0x28, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

	u8 cmd1[9] = { FTS_CMD_HW_REG_W, 0x20, 0x00, 0x00, FLASH_ERASE_CODE0 + 1, 0x00, 0xFF, 0x1C, 0x90 };
	u8 mask[6] = { 0 };
	u8 mask_cnt = 6;
#else
	...
	for (i = flash_cx_start_page; i <= flash_cx_end_page && keep_cx >= SKIP_PANEL_CX_INIT; i++) {
		logError(0, "%s Skipping erase CX page %d!\n", tag, i);
		fromIDtoMask(i, mask, mask_cnt);
	}


	for (i = flash_panel_start_page; i <= flash_panel_end_page && keep_cx >= SKIP_PANEL_INIT; i++) {
	...
	if (fts_write(cmd, ARRAY_SIZE(cmd)) < OK) {
		logError(1, "%s %s: Erase ERROR %08X\n",
			 tag, __func__, ERROR_BUS_W);
		return ERROR_BUS_W;
	}
 ...
 * @param address in memory where to copy the data, possible values
...
int flash_burn(Firmware fw, int force_burn, int keep_cx)
{
	...
	if (!force_burn) {
		for (res = EXTERNAL_RELEASE_INFO_SIZE - 1; res >= 0; res--)
			if (fw.externalRelease[res] != systemInfo.u8_releaseInfo[res])
				goto start;
		logError(1,
			 "%s %s: Firmware in the chip is the same with  firmware to flash ! NO UPDATE ERROR %08X\n",
	...
	if (force_burn == CRC_CX && fw.sec2_size == 0) {
		...
		logError(1,
			 "%s %s: CRC in CX but fw does not contain CX data! NO UPDATE ERROR %08X\n",
			 tag, __func__, ERROR_FW_NO_UPDATE);
		return ERROR_FW_NO_UPDATE | ERROR_FLASH_BURN_FAILED;
	}
...
start:
	...
	if (res < 0) {
		logError(1,
			 "%s %s: Unable to retrieve Chip INFO! ERROR %08X\n",
