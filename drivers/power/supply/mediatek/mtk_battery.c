...
static void night_charging_set_flag(int v) { }
static int input_suspend_get_flag(void) { return 0; }
static void input_suspend_set_flag(int v) { }
static void smart_batt_set_diff_fv(int v) { }
#define chr_err(fmt, ...) pr_info(fmt, ##__VA_ARGS__)
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		if (IS_ERR_OR_NULL(bs_data->chg_psy)) {
			bs_data->chg_psy = power_supply_get_by_name("mtk-master-charger");
...
int force_get_tbat_internal(struct mtk_battery *gm, bool update)
{
	...
	/* not use MTK gauge return default 25C */
	return 25;
...
static void power_misc_kthread_fgtimer_func(
	struct alarm *alarm, ktime_t now)
{
...
int battery_psy_init(struct platform_device *pdev)
{
	...
	gm->bs_data.chg_psy = power_supply_get_by_name("mtk-master-charger");
