...
static int xaga_of_update_u32(struct device_node *np, const char *name, u32 value)
{
...
static int xaga_of_update_u32_array(struct device_node *np, const char *name,
				    const u32 *values, u32 count)
{
	...
	if (!prop)
		return -ENOMEM;
	prop->name = kstrdup(name, GFP_KERNEL);
	val = kmalloc_array(count, sizeof(*val), GFP_KERNEL);
	if (!prop->name || !val) {
		kfree(prop->name);
		kfree(prop);
		kfree(val);
	...
	for (i = 0; i < count; i++)
		val[i] = cpu_to_be32(values[i]);
	prop->value = val;
	prop->length = count * sizeof(*val);

	return of_update_property(np, prop);
}

static int xaga_of_update_string(struct device_node *np, const char *name,
				 const char *value)
{
	struct property *prop;
	char *val;
...
static int xaga_of_update_gpio(struct device_node *np, const char *name,
			       struct device_node *pio, u32 pin, u32 flags)
{
	struct property *prop;
	__be32 *val;
	u32 phandle;
	...
	if (!pio)
		return -EINVAL;
	if (of_property_read_u32(pio, "phandle", &phandle))
		return -EINVAL;
	...
	if (!prop)
		return -ENOMEM;
	prop->name = kstrdup(name, GFP_KERNEL);
	val = kmalloc(3 * sizeof(*val), GFP_KERNEL);
	if (!prop->name || !val) {
		kfree(prop->name);
		kfree(prop);
		kfree(val);
	...
	val[0] = cpu_to_be32(phandle);
	val[1] = cpu_to_be32(pin);
	val[2] = cpu_to_be32(flags);
	prop->value = val;
	prop->length = 3 * sizeof(*val);

	return of_update_property(np, prop);
}

/*
 * qqcandy's effective DT keeps spi2 disabled because its child is still the
 * xaga NT36672E node. Re-enable the already-present node and repoint the
 * no-flash firmware at the Tianma NT36672C image before the platform bus
 * creates the controller. The dtbo is not rebuilt, so this has to happen at
 * runtime; see the display workaround in mtk_drm_helper_init().
 */
static void __init xaga_spi_touch_enable(void)
{
	struct device_node *spi, *child, *cfg, *pio;
	static const u32 panel_coords[] = { 4320, 9648 };
	int ret;

	spi = of_find_node_by_path("/soc@0/spi@11012000");
	if (!spi) {
		pr_err("XAGA-TP: spi2 node not found\n");
		return;
	}

	child = of_get_child_by_name(spi, "novatek");
	if (!child) {
		pr_err("XAGA-TP: novatek@0 child not found\n");
		of_node_put(spi);
		return;
	}

	ret = xaga_of_update_string(spi, "status", "okay");
	ret |= xaga_of_update_u32(child, "spi-max-frequency", 9600000);
	ret |= xaga_of_update_u32(child, "super-resolution-factor", 10);
	ret |= xaga_of_update_u32_array(child, "touchpanel,panel-coords",
					 panel_coords, ARRAY_SIZE(panel_coords));

	pio = of_find_node_by_path("/soc@0/pinctrl");
	ret |= xaga_of_update_gpio(child, "novatek,reset-gpio", pio, 212, 0);
	ret |= xaga_of_update_gpio(child, "novatek,irq-gpio", pio, 135, 0x2001);
	of_node_put(pio);

	cfg = of_get_child_by_name(child, "novatek,cfg_0");
	if (cfg) {
		ret |= xaga_of_update_string(cfg, "novatek,fw-name",
					     "FW_NF_NT36672C_TIANMA.img");
		of_node_put(cfg);
	}
	...
	if (ret) {
		pr_err("XAGA-TP: failed to patch spi2/touch properties (%d)\n", ret);
		of_node_put(spi);
		return;
	}

	pr_info("XAGA-TP: re-enabling spi2 and NT36672C touch node\n");
	of_platform_device_create(spi, NULL, NULL);
	of_node_put(spi);
}
...
void __init __no_sanitize_address setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = boot_command_line;
	 ...
	 * arch/arm64/kernel/board-dtb.o). Doing this right after
	 ...
	 */
#ifdef CONFIG_MTK_EMBED_BOARD_DTB
	extern char _binary_arch_arm64_kernel_board_embed_dtb_start[];
	extern char _binary_arch_arm64_kernel_board_embed_dtb_end[];
	if (acpi_disabled) {
		/*
		 * qqcandy: copy the embedded DTB into a writable buffer and
		 * re-inject LK's /chosen/atag,devinfo (project/panel/efuse
		 * values LK writes into the FDT it hands us). Without it the
		 * mtk-devinfo efuse cells never register and dispsys_config
		 * defers forever (6.12 PGZ110 root cause #3, HANDOFF.md).
		 * fdt_open_into also gives setprop room to work in.
		 */
		static u8 board_fdt_buf[512 * 1024] __aligned(8);
		const void *lk_fdt = initial_boot_params;
		const void *devinfo = NULL;
		int emb_size = _binary_arch_arm64_kernel_board_embed_dtb_end -
			       _binary_arch_arm64_kernel_board_embed_dtb_start;
		int lk_off, emb_off, len = 0;

		lk_off = fdt_path_offset(lk_fdt, "/chosen");
		if (lk_off >= 0)
			devinfo = fdt_getprop(lk_fdt, lk_off,
					      "atag,devinfo", &len);

		pr_info("BOARD-DTB: overriding LK FDT with embedded "
			CONFIG_MTK_EMBED_BOARD_DTB_NAME ".dtb (%d bytes)\n",
			emb_size);

		if (devinfo && len > 0 && emb_size <= sizeof(board_fdt_buf)) {
			memcpy(board_fdt_buf,
			       _binary_arch_arm64_kernel_board_embed_dtb_start,
			       emb_size);
			if (fdt_open_into(board_fdt_buf, board_fdt_buf,
					  sizeof(board_fdt_buf)) == 0) {
				emb_off = fdt_path_offset(board_fdt_buf,
							  "/chosen");
				if (emb_off >= 0)
					fdt_setprop(board_fdt_buf, emb_off,
						    "atag,devinfo",
						    devinfo, len);
				initial_boot_params = board_fdt_buf;
			} else {
				initial_boot_params =
					_binary_arch_arm64_kernel_board_embed_dtb_start;
			}
		 ...
		 * OUR embedded FDT so cmdline changes actually take effect.
		 * saved_command_line is built later in start_kernel, so this
		...
		if (!strstr(boot_command_line, "pd_ignore_unused"))
			strncat(boot_command_line, " pd_ignore_unused",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * Bring-up: full verbosity (v22+ calmed the boot; conninfra
		 * wait is bounded since v28). MUST stay above
		 * CONSOLE_LOGLEVEL_QUIET(4): fbcon_init hides the Tux boot
		 * logo whenever console_loglevel <= 4 at bind time.
		 */
		if (!strstr(boot_command_line, "loglevel="))
			strncat(boot_command_line, " loglevel=7",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * Bring-up: never auto-reboot on panic. A frozen screen IS
		 * the crash dump for a UART-less device; remove once M2/M3
		 * stabilize.
		 */
		/* unconditional + last: later panic= wins, overriding
		 * anything LK put in the cmdline. */
		strncat(boot_command_line, " panic=-1",
			COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * XAGA: vendor_boot's ramdisk is concatenated AFTER ours
		 * and its Android /init wins the filename collision (three
		 * camera-verified boots prove it). Our /xinit (gadget +
		 * rootfs script) ships as a real entry in the boot ramdisk
		 * cpio; rdinit=/xinit picks it regardless of concat order.
		 */
		if (!strstr(boot_command_line, "rdinit="))
			strncat(boot_command_line, " rdinit=/xinit",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * v62: systemd re-enabled. The UFS hibern8 bug (root cause
		 * of the v54 systemd early hang) is fixed in v61; no more
		 * init=/bin/sh. If a bring-up shell is needed again, append
		 * init=/bin/sh here temporarily.
		 */
		/*
		 * XAGA: expdb wdt_status 0x6 == SPMWDT|SPMWDT_EXT - the
		 * resets are the SPM-domain watchdog, not the AP WDT at
		 * 1c007000. Classic bring-up signature: first deep-idle /
		 * SPM-handshake transition after boot settles bites. Stay
		 * out of idle states entirely until the SPM leg is proven;
		 * keep the console awake while we're at it.
		 */
		if (!strstr(boot_command_line, "cpuidle.off="))
			strncat(boot_command_line, " cpuidle.off=1",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		if (!strstr(boot_command_line, "consoleblank="))
			strncat(boot_command_line, " consoleblank=0",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * v83: THP re-enabled together with the memfd lru_add_drain_all
		 * revert; if systemd wedges again, bring back both together.
		 */
		/* v99: fbdev emulation re-enabled — dual_pipe Oops fixed in v70 */
		/*
		 * XAGA bring-up: LK leaves the HW watchdog armed and
		 * mtk_wdt re-arms 31 s at probe; nothing on a pmOS rootfs
		 * feeds it, so the board silently resets ~31 s after boot
		 * looks "hung". Kill the counter now; bring-up wants a
		 * frozen readable screen over auto-recovery anyway.
		 * (MTK WDT: MODE reg 0x00, write KEY with EN=0.)
		 */
		/*
		 * XAGA: WDT strategy changed (v38) - feed, don't fight.
		 * The watchdog node is enabled again and the core's
		 * handle_boot_enabled pinger keeps it satisfied; the
		 * setup_arch-time register kills never demonstrably took
		 * effect (their prints die before the expdb console window
		 * opens) and may even be dropped by a secure filter.
		 */
		/*
		 * v68: do NOT append console=ttyS0. Registering ttyS0 as a
		 * CON_PRINTBUFFER console flushed ~200KB of early boot log
		 * at 115200 baud and cost 19 s (suspense #3 first item).
		 * The UART itself still probes in ~50 ms; debug via ttyGS0/
		 * RNDIS instead.
		 */
		/*
		 * USB bring-up (v48): fw_devlink=off UNCONDITIONALLY. With
		 * the default fw_devlink=on, usb0's role-switch endpoint
		 * creates an fwnode dependency on the usb-c-connector under
		 * mt6375@34 (status okay, but the MT6375 MFD/tcpc stack is
		 * not ported yet and never probes) -> usb0 sat in the
		 * deferred queue forever and its probe never ran (v18 [PROBE]
		 * marker evidence). __setup handlers run per occurrence and
		 * the last one wins, so appending here also overrides any
		 * fw_devlink= value LK injected (the v19 guard left that
		 * hole; v19 itself was rolled back untested).
		 */
		strncat(boot_command_line, " fw_devlink=off",
			COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * v51: console=ttyGS0 is POISON until the gadget is bound
		 * to a UDC AND a host is attached. The console registers
		 * at boot; the moment mkdir functions/acm.GS0 creates the
		 * ttyGS0 device the console attaches and printk flushes its
		 * backlog into a host-less ACM port -> the write blocks
		 * holding console_lock -> every legacy console (VT/fbcon
		 * screen, kmsg) freezes (v49/v50 evidence: boot stalls at
		 * "printk: legacy console [ttyGS0]", init output dies right
		 * after "linked acm.GS0"). Host-side serial comes back via
		 * userspace getty on ttyACM0 once USB is proven.
		 */
		/*
		 * Console visibility (v48): /dev/console routes to the LAST
		 * console= entry, and opening it failed outright when no tty
		 * console was alive ("Warning: unable to open an initial
		 * console" -- ttyGS0 never enables without the UDC, ttyS0
		 * likewise). The VT console IS a device console
		 * (.device = vt_console_device), so console=tty0 makes
		 * /dev/console the fbcon screen: console_on_rootfs() gets
		 * init real stdio and userspace output becomes visible.
		 * Must stay the LAST console= appended.
		 */
		if (!strstr(boot_command_line, "console=tty0"))
			strncat(boot_command_line, " console=tty0",
				COMMAND_LINE_SIZE - strlen(boot_command_line) - 1);
		/*
		 * LK passes mtk_printk_ctrl.disable_uart=1, which kills the
		 * g_serial console a few seconds after boot and hides all
		 * bring-up logs. Flip it to 0 so ttyGS0 keeps streaming
		 * (6.12 PGZ110 debugging hack, p[29] = the '1' of "=1").
		 */
		{
			char *p = strstr(boot_command_line,
